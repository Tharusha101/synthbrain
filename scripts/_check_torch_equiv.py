...
def main():
    ...
    in_spikes = encoding.poisson_encode(
        img, T, dt=1.0, max_rate=eff, rng=np.random.default_rng(123)
    )  # (T, n_in) bool

    # --- NumPy forward (no learning) ---
    net.exc.reset_state()
    net.exc.theta[:] = theta0
    net.exc_syn.reset_state()
    net.inh_syn.reset_state()
    ...
    print(
        f"per-neuron count diff:   max={diff.max()}  mean={diff.mean():.4f}  "
        f"neurons differing={int((diff > 0).sum())}/{n_exc}"
    )
    ok = diff.max() == 0
    print(
        "FORWARD DYNAMICS MATCH (exact)"
        if ok
        else (
            "forward dynamics differ -- investigate"
            if diff.max() > 2
            else "forward dynamics match within +/-2 spikes (float32/ordering at threshold)"
        )
    )
    ...
    print(
        ...
        f"(~{emp_torch:.1f} Hz/active pixel)"
    )
