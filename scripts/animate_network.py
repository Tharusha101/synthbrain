...
def get_net_and_data(rng):
    ...
    try:
        ...
        print(
            f"[anim] MNIST unavailable ({e.__class__.__name__}); using synthetic digits."
        )
...
def main():
    ...
    gs = fig.add_gridspec(
        2, 2, width_ratios=[1, 3.0], height_ratios=[1, 1], hspace=0.35, wspace=0.22
    )
    ...
    def update(t_global):
        ...
        if seg != state["seg"]:  # new digit: switch image, clear retina
            ...
            ax_img.set_title(
                f"input digit (true = {segments[seg]['label']})", fontsize=10
            )
        ...
        mask = ts_g <= t_global
        scat.set_offsets(
            np.c_[ts_g[mask], ns_g[mask]] if mask.any() else np.empty((0, 2))
        )
        ...
        verdict = (
            "?"
            if p < 0
            else f"{p} " + ("(correct)" if p == segments[seg]["label"] else "(wrong)")
        )
        ras_title.set_text(
