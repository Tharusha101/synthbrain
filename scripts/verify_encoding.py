...
def rate_scaling():
    ...
    assert np.allclose(
        emp_rate, expected, atol=3.0
    ), "empirical rate off expected by >3 Hz"
    return emp_rate
...
def raster():
    ...
    ax.set(
        xlabel="time (ms)",
        ylabel="neuron (low->high intensity)",
    ...
    assert (
        counts[-1] > counts[0]
    ), "high-intensity neuron did not out-fire low-intensity one"
    print(
        f"[raster] low-intensity spikes={counts[0]}, high-intensity spikes={counts[-1]}"
    )
    return True
