...
def epsp_trace():
    ...
    print(f"[epsp] peak={peak:.3f} (want {w}), decay ratio={ratio:.4f} (want {expected:.4f})")
    ...
    ax.set(xlabel="time (ms)", ylabel="synaptic current g", title="EPSP: single-spike synaptic current")
...
def normalization():
    ...
    print(f"[normalize] column sums in [{sums.min():.3f}, {sums.max():.3f}] (want 78.0)")
